...
def cells(ue):
    ...
    cells = []
    if df is not None:
        nbc = db.data.filter(regex="Nb1_id").loc[[0],["Viavi_Nb1_id"]].values[0].tolist()
        nbc = nbc + db.data.filter(regex="Nb2_id").loc[[0],["Viavi_Nb2_id"]].values[0].tolist()
        srvc = db.data.filter(regex="Cell_id").values[0].tolist()
        cells = srvc+nbc
        # logger.info("Cells: {}".format(cells))
    return cells
    ...
    for ueid in ue_list:
        ...
        for cid in cell_list:
            ...
            if db.data is not None and len(db.data) != 0:
                try:
                    inp = (db.data[db.thptparam]) * 1000 # new
